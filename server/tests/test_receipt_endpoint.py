...
class ReceiptEndpointTests(unittest.TestCase):
    ...
    def test_creation_returns_persisted_number_totals_and_items(self):
        response = self.post(self.payload())
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.headers['cache-control'], 'no-store')
        self.assertEqual(set(response.json()), {'data'})
        data = response.json()['data']
        self.assertIsNone(data['source_invoice_id'])
        ...
        item = data['items'][0]
        self.assertEqual(item['receipt_id'], data['id'])
        self.assertEqual(item['line_total'], '100.00')
        self.assertEqual(self.connection.execute('SELECT receipt_number FROM receipts WHERE id = %s',
                                                (UUID(data['id']),)).fetchone()[0], 'RCT-0001')
        self.assertEqual(self.post(self.payload()).json()['data']['receipt_number'], 'RCT-0002')
    ...
    def test_invalid_and_forged_fields_never_create_rows(self):
        ...
        payloads += [self.payload(**{field: 'forged'}) for field in
                     ('user_id', 'source_invoice_id', 'receipt_number', 'subtotal', 'tax_amount', 'discount_amount', 'total', 'status', 'terms', 'due_date')]
    ...
    def test_authentication_required(self):
        ...
        for headers in ({}, {'Authorization': 'Bearer invalid'}, {'Authorization': 'Bearer ' + refresh}):
            response = self.client.post('/api/v1/receipts', json=self.payload(), headers=headers)
    ...
    def test_item_database_failure_rolls_back_and_returns_safe_error(self):
        self.connection.execute('''CREATE FUNCTION reject_endpoint_item() RETURNS TRIGGER LANGUAGE plpgsql AS $$
            ...
            CREATE TRIGGER reject_endpoint_item BEFORE INSERT ON receipt_items
    ...
    def test_openapi_documents_authenticated_creation(self):
        path = self.client.get('/openapi.json').json()['paths']['/api/v1/receipts']
    ...
    def test_source_quote_link_is_rejected_even_when_null(self):
        for value in (None, str(uuid4())):
            with self.subTest(value=value):
                response = self.post(self.payload(source_invoice_id=value))
                self.assertEqual(response.status_code, 422)
        self.assertEqual(self.connection.execute('SELECT count(*) FROM receipts').fetchone()[0], 0)
