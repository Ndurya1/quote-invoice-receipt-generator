...
class QuoteNumberingTests(unittest.TestCase):
    ...
    def test_upgrade_seeds_existing_quote_numbers(self):
        ...
        try:
            ...
            for number in ('QT-0003', 'QT-0012', 'LEGACY-99'):
            ...
            self.assertEqual(apply_migrations(self.connection), ['002_quote_numbering.sql'])
            self.assertEqual(next_quote_number(self.connection, user_id=owner.id), 'QT-0013')
        finally:
    ...
    def test_sequences_are_independent_and_expand_past_four_digits(self):
        ...
        self.assertEqual(next_quote_number(self.connection, user_id=first.id), 'QT-0001')
        self.assertEqual(next_quote_number(self.connection, user_id=first.id), 'QT-0002')
        self.assertEqual(next_quote_number(self.connection, user_id=second.id), 'QT-0001')
        self.connection.execute('UPDATE quote_number_counters SET last_number = 9999 WHERE user_id = %s', (first.id,))
        self.assertEqual(next_quote_number(self.connection, user_id=first.id), 'QT-10000')
    ...
    def test_concurrent_first_allocations_are_unique(self):
        ...
        def allocate(_):
            with connect_database(self.settings, test=True) as connection:
                ...
                return next_quote_number(connection, user_id=owner.id)

        with ThreadPoolExecutor(max_workers=6) as executor:
            numbers = list(executor.map(allocate, range(6)))
        self.assertEqual(sorted(numbers), [f'QT-{i:04d}' for i in range(1, 7)])

    def test_rollback_restores_both_new_and_existing_counter(self):
        owner = self.owner()
        for expected in ('QT-0001', 'QT-0002'):
            with self.assertRaisesRegex(RuntimeError, 'abort'):
                with self.connection.transaction():
                    self.assertEqual(next_quote_number(self.connection, user_id=owner.id), expected)
    ...
    def test_persisted_number_is_immutable_and_deletion_does_not_reuse_it(self):
        ...
        with self.connection.transaction():
            number = next_quote_number(self.connection, user_id=owner.id)
        ...
        with self.assertRaises(errors.CheckViolation):
            self.connection.execute("UPDATE quotes SET quote_number = 'QT-9999' WHERE id = %s", (quote_id,))
        self.connection.execute("UPDATE quotes SET notes = 'Updated' WHERE id = %s", (quote_id,))
        self.assertEqual(self.connection.execute('SELECT quote_number FROM quotes WHERE id = %s', (quote_id,)).fetchone()[0], number)
