...
class ReceiptSchemaTests(unittest.TestCase):
    def test_valid_request_defaults_and_explicit_fields(self):
        request = ReceiptCreate(**receipt_payload())
        ...
        full = ReceiptCreate(**receipt_payload(tax_rate='16.000',
    ...
    def test_required_fields_empty_items_and_dates(self):
        ...
        for field in ('client_id', 'issue_date', 'currency', 'items'):
            with self.subTest(field=field), self.assertRaises(ValidationError):
                ReceiptCreate(**{k: v for k, v in payload.items() if k != field})
        for changes in ({'items': []}, {'items': None}, {'client_id': 'bad'},
            ...
            with self.subTest(changes=changes), self.assertRaises(ValidationError):
                ReceiptCreate(**receipt_payload(**changes))
    ...
    def test_nested_item_validation_and_computed_field_tampering(self):
        for changes in ({'description': ' '}, {'quantity': '0'}, {'quantity': '0.0001'},
            ...
            with self.subTest(changes=changes), self.assertRaises(ValidationError):
                ReceiptCreate(**receipt_payload(items=[{'description': 'Work', 'quantity': '1', 'unit_price': '1', **changes}]))
        for field in ('id', 'user_id', 'source_invoice_id', 'receipt_number', 'line_total', 'subtotal', 'tax_amount',
                      'discount_amount', 'total', 'status', 'created_at', 'updated_at', 'expiry_date', 'terms'):
            with self.subTest(field=field), self.assertRaises(ValidationError):
                ReceiptCreate(**receipt_payload(**{field: 'forged'}))


class ReceiptCreationValidationTests(unittest.TestCase):
    ...
    def validate(self, **changes):
        payload = ReceiptCreate(**receipt_payload(**{'client_id': self.client_id, **changes}))
        return validate_receipt_create(self.connection, user_id=self.owner.id, payload=payload)
