...
class InvoiceSendTests(unittest.TestCase):
    ...
    def send(self, invoice_id, token=None):
    ...
    def test_invalid_repeated_foreign_missing_and_unauthenticated_requests(self):
        invoice = self.post(self.payload()).json()['data']
        self.assertEqual(self.send(invoice['id']).status_code, 200)
        repeated = self.send(invoice['id'])
        ...
        self.assertEqual(self.client.post(f"/api/v1/invoices/{invoice['id']}/send").status_code, 401)
        ...
        foreign = self.send(invoice['id'], token=other_token)
