...
class InvoiceEndpointTests(unittest.TestCase):
    ...
    def test_creation_returns_persisted_draft_number_totals_and_items(self):
        ...
        self.assertIsNone(data['source_quote_id'])
        ...
        item = data['items'][0]
        self.assertEqual(item['invoice_id'], data['id'])
        self.assertEqual(item['line_total'], '100.00')
        self.assertEqual(self.connection.execute('SELECT invoice_number FROM invoices WHERE id = %s',
                                                (UUID(data['id']),)).fetchone()[0], 'INV-0001')
        self.assertEqual(self.post(self.payload()).json()['data']['invoice_number'], 'INV-0002')

    def test_invalid_and_forged_fields_never_create_rows(self):
        payloads = [self.payload(items=[]), self.payload(currency='kes'), self.payload(due_date='2026-09-14')]
        payloads += [self.payload(**{field: 'forged'}) for field in
                     ('user_id', 'source_quote_id', 'invoice_number', 'subtotal', 'tax_amount', 'discount_amount', 'total', 'status')]
    ...
    def test_authentication_required(self):
        ...
        for headers in ({}, {'Authorization': 'Bearer invalid'}, {'Authorization': 'Bearer ' + refresh}):
            response = self.client.post('/api/v1/invoices', json=self.payload(), headers=headers)
    ...
    def test_item_database_failure_rolls_back_and_returns_safe_error(self):
        self.connection.execute('''CREATE FUNCTION reject_endpoint_item() RETURNS TRIGGER LANGUAGE plpgsql AS $$
            ...
            CREATE TRIGGER reject_endpoint_item BEFORE INSERT ON invoice_items
    ...
    def test_openapi_documents_authenticated_creation(self):
        path = self.client.get('/openapi.json').json()['paths']['/api/v1/invoices']
        self.assertEqual(set(path), {'post'})
        self.assertEqual(path['post']['security'], [{'HTTPBearer': []}])
        self.assertIn('201', path['post']['responses'])

    def test_source_quote_link_is_rejected_even_when_null(self):
        for value in (None, str(uuid4())):
            with self.subTest(value=value):
                response = self.post(self.payload(source_quote_id=value))
                self.assertEqual(response.status_code, 422)
        self.assertEqual(self.connection.execute('SELECT count(*) FROM invoices').fetchone()[0], 0)
