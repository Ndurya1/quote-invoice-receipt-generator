...
def next_receipt_number(connection: Connection, *, user_id: UUID) -> str:
    """Call inside receipt creation's transaction to roll back allocation with it."""
    with connection.transaction():
        row = connection.execute(
            '''INSERT INTO receipt_number_counters (user_id, last_number) VALUES (%s, 1)
               ON CONFLICT (user_id) DO UPDATE
               SET last_number = receipt_number_counters.last_number + 1
               WHERE receipt_number_counters.last_number < 9223372036854775807
               RETURNING last_number''',
            (user_id,),
        ).fetchone()
        if row is None:
            raise DomainError('RECEIPT_NUMBER_EXHAUSTED', 'Receipt number range is exhausted.', status_code=409)
        return f'RCT-{row[0]:04d}'
