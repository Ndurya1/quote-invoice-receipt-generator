...
import { landingPrimaryAction, shouldShowPublicAuthActions } from '../features/landing/landingAccountActions.js';
...
export default function LandingPage() {
  ...
  const primaryAction = landingPrimaryAction(status);
  const showPublicAuthActions = shouldShowPublicAuthActions(status);
  const heroActionLabel = status === 'ready' ? 'Go to dashboard' : status === 'needsOnboarding' ? 'Finish setup' : 'Create your first quotation';
  return <div id="home">
    <a className="skip-link" href="#main-content">Skip to content</a><Header />
    <main id="main-content" tabIndex={-1}>
      <section className="hero container" aria-labelledby="hero-title"><div className="hero-copy"><p className="eyebrow">Quotations <ArrowRight size={14} aria-hidden="true" /> Invoices <ArrowRight size={14} aria-hidden="true" /> Receipts</p><h1 id="hero-title">Create it once.<br /><span>Keep the paperwork moving.</span></h1><p className="hero-description">Create a professional quotation, then carry the details into an invoice and receipt. No starting over.</p><div className="hero-actions"><Link className="button" to={primaryAction.to}>{heroActionLabel} <ArrowRight size={16} aria-hidden="true" /></Link><a className="button button-secondary" href="#how-it-works">See how it works</a></div><p className="helper">Set up your business details once. Reuse them on your next document.</p></div><InvoiceDemo /></section>
      ...
      <section className="container final-section" aria-labelledby="final-title"><div className="final-cta"><p className="eyebrow">Ready when you are</p><h2 id="final-title">Create the document.<br />Reuse the work.</h2><p>Set up your business details once and create your first quotation.</p><Link className="button" to={primaryAction.to}>{status === 'ready' ? 'Go to dashboard' : status === 'needsOnboarding' ? 'Finish setup' : 'Get started'} <ArrowRight size={16} aria-hidden="true" /></Link>{showPublicAuthActions && <Link className="text-button" to={routePaths.login}>Log in</Link>}</div></section>
    </main>
    <footer className="container public-footer"><div><Brand /><p>Connected quotations, invoices and receipts<br />for people doing real work.</p><p className="copyright">© {new Date().getFullYear()} DocuFlow</p></div><nav aria-label="Footer navigation"><a href="#how-it-works">How it works</a><a href="#benefits">Benefits</a><Link className="text-button" to={routePaths.privacy}>Privacy Policy</Link><Link className="text-button" to={routePaths.terms}>Terms of Service</Link>{showPublicAuthActions && <><Link className="text-button" to={routePaths.login}>Log in</Link><Link className="text-button" to={routePaths.register}>Create account</Link></>}</nav></footer>
  </div>;
}
