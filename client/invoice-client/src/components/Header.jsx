...
export default function Header() {
  ...
  return <header className="public-header" ref={root}>
    <nav className="container nav-row" aria-label="Main navigation">
      ...
      <div className="account-actions">{showPublicAuthActions && <Link className="text-button" to={routePaths.login}>Log in</Link>}<Link className="button nav-start" to={primaryAction.to}>{primaryAction.label}</Link>
      ...
      <div id="mobile-navigation" className="mobile-links" hidden={!open}><a href="#how-it-works" onClick={() => setOpen(false)}>How it works</a><a href="#benefits" onClick={() => setOpen(false)}>Benefits</a><Link className="button" to={primaryAction.to} onClick={() => setOpen(false)}>{primaryAction.label}</Link>{showPublicAuthActions && <Link className="text-button" to={routePaths.login} onClick={() => setOpen(false)}>Log in</Link>}</div>
    </nav>
  </header>;
}
