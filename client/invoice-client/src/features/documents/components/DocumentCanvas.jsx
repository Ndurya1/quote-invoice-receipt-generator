...
export default function DocumentCanvas({ document, client, businessProfile, type = 'quotation' }) {
  ...
  return <article className="quotation-canvas" aria-labelledby="document-canvas-title">
    <header className="quotation-canvas__header">
      ...
      <div className="quotation-canvas__metadata"><div><h3>{isInvoice ? 'Invoice number' : 'Quotation number'}</h3><span>{number}</span></div><div><h3>Date of issue</h3><span>{formatDate(document.issue_date)}</span></div><div><h3>{secondaryDateLabel}</h3><span>{formatDate(secondaryDate)}</span></div></div>
    ...
    <section className="quotation-canvas__body">
      ...
      <div className="quotation-canvas__bottom"><div className="quotation-canvas__terms">{document.terms && <div><h3>Terms</h3><p>{document.terms}</p></div>}{document.notes && <div><h3>Notes</h3><p>{document.notes}</p></div>}</div><dl className="quotation-canvas__totals"><div><dt>Subtotal</dt><dd>{formatCurrency(document.subtotal, document.currency)}</dd></div><div><dt>Tax rate</dt><dd>{`${document.tax_rate || '0.000'} %`}</dd></div><div><dt>Tax</dt><dd>{formatCurrency(document.tax_amount, document.currency)}</dd></div>{document.discount_amount && document.discount_amount !== '0.00' && <div><dt>Discount</dt><dd>− {formatCurrency(document.discount_amount, document.currency)}</dd></div>}<div className="quotation-canvas__grand-total"><dt>{isInvoice ? 'Invoice total' : 'Quotation total'}</dt><dd>{formatCurrency(document.total, document.currency)}</dd></div></dl></div>
    </section>
