...
import { getPreviewClient, listPreviewClients } from './clientPreview.js';

function isAbortError(error) {
  return error?.name === 'AbortError';
}

export function useClientsList(query) {
  const { user } = useAuth();
  const cache = useDataCache();
  const [state, setState] = useState({ status: 'loading', data: null, error: null });
  const requestId = useRef(0);
  const queryKey = queryKeys.clientsList(query);
  const load = useCallback(async (signal) => {
    ...
    try {
      const result = isAuthPreviewEnabled ? listPreviewClients(query) : await listClients(query, undefined, { signal });
...
export function useClient(clientId) {
  ...
  const [state, setState] = useState({ status: 'loading', data: null, error: null });
  const requestId = useRef(0);
  const queryKey = queryKeys.clientDetail(clientId);
  const load = useCallback(async (signal) => {
    const currentRequest = requestId.current + 1;
    requestId.current = currentRequest;
    setState({ status: 'loading', data: null, error: null });
    ...
    try {
      const result = isAuthPreviewEnabled ? getPreviewClient(clientId) : await getClient(clientId, undefined, { signal });
      if (currentRequest !== requestId.current) return;
      cache.set(queryKey, result);
      setState({ status: 'success', data: result, error: null });
    } catch (error) {
